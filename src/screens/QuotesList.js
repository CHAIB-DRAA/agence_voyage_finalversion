...
export default function QuotesList({ navigation, route }) {
  ...
  const [updatingId, setUpdatingId] = useState(null); // Pour éviter le double-clic sur le switch
  ...
  // --- NOUVELLE FONCTION : BASCULE DU STATUT ---
  const toggleProcessingStatus = async (item) => {
    if (updatingId === item.id || updatingId === item._id) return; // Anti-spam
  ...
  const getStatusConfig = (status) => {
    switch (status) {
      ...
      default: return { color: '#F39C12', label: 'في الانتظار (En attente)', bg: 'rgba(243, 156, 18, 0.1)', icon: 'clock' };
    }
  };
          ...
          <View style={styles.cardHeader}>
            ...
            <View style={{flexDirection:'row', alignItems:'center', gap: 8}}>
                ...
                <View style={styles.switchContainer}>
                    <Text style={[styles.switchLabel, {color: isTraite ? '#2ECC71' : '#888'}]}>
                        {isTraite ? 'Traité' : 'À traiter'}
                    </Text>
                    <Switch
                        trackColor={{ false: "#3e3e3e", true: "rgba(46, 204, 113, 0.3)" }}
                        thumbColor={isTraite ? "#2ECC71" : "#f4f3f4"}
                ...
                {total > 0 && (
                    <View style={[styles.paymentBadge, { backgroundColor: isPaid ? '#2ECC71' : '#E74C3C' }]}>
                        <Text style={styles.paymentText}>{isPaid ? 'Payé' : 'Dû'}</Text>
                    </View>
                )}
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  switchContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.2)', borderRadius: 20, paddingHorizontal: 8, paddingVertical: 2 },
